...
export default function Hero() {
  return (
    <section style={{ backgroundColor: "#2D1400", paddingTop: 64, position: "relative", overflow: "hidden", minHeight: "92vh", display: "flex", alignItems: "center" }}>
      {/* Background texture */}
      <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(circle at 70% 50%, rgba(232,115,10,0.12) 0%, transparent 60%)" }} />
      <div style={{ position: "absolute", top: 0, right: 0, width: "50%", height: "100%", overflow: "hidden" }} className="hide-sp">
        <img src={`${IMG}/top/image.png`} alt="ひだまり整骨院 正雀院" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", opacity: 0.35 }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, #2D1400 0%, transparent 40%)" }} />
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "80px 28px", position: "relative", zIndex: 1, width: "100%" }}>
        <div style={{ maxWidth: 620 }}>
          ...
          <p style={{ fontSize: "clamp(13px, 1.4vw, 15px)", color: "rgba(255,255,255,0.7)", lineHeight: 2.1, marginBottom: 40, maxWidth: 480 }}>
          ...
          <div style={{ display: "flex", gap: 0, borderTop: "1px solid rgba(255,255,255,0.12)" }}>
            {[
              ...
              <div key={label} style={{ flex: 1, padding: "18px 8px 0", borderRight: i < 3 ? "1px solid rgba(255,255,255,0.12)" : "none", paddingLeft: i === 0 ? 0 : 12 }}>
                <div style={{ fontSize: 13, fontWeight: 900, color: "#E8730A", marginBottom: 4 }}>{num}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", lineHeight: 1.5 }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
