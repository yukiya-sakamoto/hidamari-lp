      ...
      <main style={{ backgroundColor: "#fff", padding: "80px 28px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48 }} className="grid-2">
            ...
            {/* 情報 */}
            <div>
              <div style={{ fontSize: 24, fontWeight: 900, color: "#0F2018", marginBottom: 4 }}>ひだまり整骨院 正雀院</div>
              <div style={{ fontSize: 13, color: "#3D6A52", marginBottom: 32 }}>大阪府摂津市正雀本町１丁目20-30</div>

              <div style={{ borderTop: "1px solid #C5DBCF", marginBottom: 32 }}>
                {[
                  ["最寄り駅", "阪急 正雀駅より徒歩4分 / JR 岸辺駅より徒歩10分"],
                  ["駐車場", "2台完備・駐車料金は当院が負担"],
                  ["保険", "各種保険・交通事故・労災保険取扱"],
                ].map(([label, val]) => (
                  <div key={label} style={{ display: "flex", gap: 20, borderBottom: "1px solid #C5DBCF", padding: "16px 0", alignItems: "flex-start" }}>
                    <span style={{ fontSize: 10, color: "#2D6A4F", fontWeight: 700, width: 56, flexShrink: 0, paddingTop: 2, letterSpacing: "0.04em" }}>{label}</span>
                    <span style={{ fontSize: 13, color: "#0F2018", lineHeight: 1.7 }}>{val}</span>
                  </div>
                ))}
              </div>

              <div style={{ backgroundColor: "#F0F7F3", border: "1px solid #C5DBCF", padding: "20px 24px", marginBottom: 24 }}>
                <div style={{ fontSize: 10, color: "#2D6A4F", fontWeight: 700, marginBottom: 14, letterSpacing: "0.12em" }}>受付時間</div>
                {[
                  ["月〜金", "9:00〜12:00 / 15:00〜20:00"],
                  ["土曜", "9:00〜12:00（午後休）"],
                  ["日・祝", "休診"],
                ].map(([day, time]) => (
                  <div key={day} style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #C5DBCF", padding: "11px 0" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <a href="https://maps.google.com/maps?q=大阪府摂津市正雀本町１丁目20-30" target="_blank" rel="noopener noreferrer"
                  style={{ display: "block", border: "1px solid #C5DBCF", color: "#2D6A4F", textAlign: "center", padding: "13px", textDecoration: "none", fontSize: 13, fontWeight: 700, borderRadius: 2 }}>
                  Google マップで見る
                </a>
              </div>
            </div>
